...
const VOICE_SETS = {
  ...
  /**
   * Bulbul v3 ships 39 voices. `shubh` is the model's own default and the
   * safest male voice; the pairs beside it are picked to be distinguishable
   * from each other rather than to be individually pretty, which is the only
   * property that matters in a two host format.
   */
  sarvam: {
    warm: { female: "kavya", male: "shubh" },
    bright: { female: "priya", male: "aditya" },
    calm: { female: "ritu", male: "rahul" },
  },
} as const;
...
/**
 * Left unset, the voice names follow the provider. That is right every time
 * except one: an OpenAI-shaped client pointed at Kokoro, which speaks the same
 * API under a different set of names.
 */
const set = VOICE_SETS[env.TTS_VOICES ?? (env.TTS_PROVIDER === "sarvam" ? "sarvam" : "openai")];
...
export type SpeakOptions = {
  /** How to say it, in prose. Only the steerable OpenAI models read this. */
  instructions?: string;
  /** Which language the text is in. Backends with real per-language voices need it. */
