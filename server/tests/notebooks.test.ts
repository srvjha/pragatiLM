...
async function createNotebook(name?: string): Promise<NotebookDto> {
  const response = await agent.post("/api/notebooks").send(name === undefined ? {} : { name });
...
describe("POST /api/notebooks", () => {
  ...
  it("rejects a name over 80 characters with a field level detail", async () => {
    const response = await agent.post("/api/notebooks").send({ name: "x".repeat(81) });
...
describe("GET /api/notebooks", () => {
  it("returns an empty list rather than 404 when there are none", async () => {
    const response = await agent.get("/api/notebooks");
...
describe("GET /api/notebooks/:notebookId", () => {
  it("returns the notebook", async () => {
    ...
    const response = await agent.get(`/api/notebooks/${created.id}`);
  ...
  it("404s for an id that does not exist", async () => {
    const response = await agent.get(`/api/notebooks/${MISSING_ID}`);
  ...
  it("400s for an id that is not a uuid, rather than reaching the database", async () => {
    expect((await agent.get("/api/notebooks/not-a-uuid")).status).toBe(400);
  });
});
...
describe("PATCH /api/notebooks/:notebookId", () => {
  it("renames and advances updatedAt", async () => {
    ...
    const response = await agent.patch(`/api/notebooks/${created.id}`).send({ name: "After" });
  ...
  it("rejects an empty name", async () => {
    ...
    expect((await agent.patch(`/api/notebooks/${created.id}`).send({ name: "" })).status).toBe(400);
  });

  it("404s before validating the body, for a notebook that does not exist", async () => {
    expect(
      (await agent.patch(`/api/notebooks/${MISSING_ID}`).send({ name: "Whatever" })).status,
    ).toBe(404);
  });
});
...
describe("DELETE /api/notebooks/:notebookId", () => {
  it("deletes and cascades to sources, per FR-1.4", async () => {
    ...
    expect((await agent.delete(`/api/notebooks/${created.id}`)).status).toBe(204);
  ...
  it("404s when deleting twice", async () => {
    ...
    await agent.delete(`/api/notebooks/${created.id}`);
    expect((await agent.delete(`/api/notebooks/${created.id}`)).status).toBe(404);
  });
});
...
describe("cross notebook access", () => {
  it("404s a source requested under the wrong notebook, rather than 200", async () => {
    ...
    const response = await agent.get(`/api/notebooks/${other.id}/sources/${source?.id}`);
    expect(response.status).toBe(404);
  });
});
...
describe("error envelope", () => {
  ...
  it("never returns a { data } key alongside an error", async () => {
    expect(await agent.get(`/api/notebooks/${MISSING_ID}`)).not.toHaveProperty("body.data");
  });
});
